	...
	public abstract class ExpandableInspectorField : InspectorField
	{
		...
		protected InspectorField CreateDrawerForComponent( Component component, string variableName = null )
		{
			...
			if( variableDrawer != null )
			{
				...
				string[,] var = Inspector.PropertyController.CheckVariables(component.gameObject);
				string name;
				if(var.Length>0 && var[0,1]!="")
					name= var[0,1];
				else
				{
					object[] attributes = component.GetType().GetCustomAttributes( typeof( rvPlanner ), true );
					if(attributes.Length>0)
						name = ((rvPlanner)attributes[0]).Name;
					else
				...
				if( variableName == null )
					// realvirtual
					if (Inspector.ShowTypeOfGameObjects)
						variableName = name + " component";
					else
					{
						variableName = name;
					}
...
#if REALVIRTUAL_PLANNER
			Inspector.PropertyController.CheckButtonAreaNeeded(component);
#endif
